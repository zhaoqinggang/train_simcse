...
tokenizer_name_path = '/home/jovyan/bge-large-zh-v1.5'
max_length = 32
split = 0.1
############################################################################
model_version ='bike_data'
...
#         print(f"处理索引: {index}")  # 打印当前处理的索引
        query = self.data.loc[index, 'query']
        label = self.data.loc[index, 'label']
#         print(f"查询: {query}, 标签: {label}")  # 打印查询和标签

        encoded_input = self.tokenizer(query, max_length=self.max_length, padding='max_length', truncation=True, return_tensors='pt')

#         print(f"编码后的输入大小: {encoded_input['input_ids'].shape}")  # 打印编码后的输入大小
...
class SimCSE(nn.Module):
    def __init__(self, model_name_path,hidden_size, num_classes,dropout_rate=0.):
        ...
        self.bert_model2 = BertModel.from_pretrained(model_name_path)


    def forward(self, input_ids, attention_mask,type0=0):
        if type0==1:
            outputs = self.bert_model2(input_ids=input_ids, attention_mask=attention_mask)
...
#         print(embeddings.shape)
        ...
        for _,source_faqs in enumerate(faqs_loader):
            ...
            faqs_embeddings = model(faqs_input_ids, faqs_attention_mask,1)
    ...
    # 评估
        if batch_idx %50  == 0:
            ...
            if val_accuracy >= best_val_accuracy:
                ...
                SAVE_PATH = pre_path+'/pipei'+model_version+'2222.pt'
...
if __name__ == '__main__':
   ...
   #===============预处理模型和数据================================================================
    ...
    faqs_loader = DataLoader(faqs_dataset, batch_size=4)
